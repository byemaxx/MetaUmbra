...
def _unit_combination_pvalues(
    ...
    *,
    ...
    return {
        "simes-intersection": _clip_pvalue(
            simes_intersection_p_2(p_unique=p_unique, p_shared=p_shared)
        ),
        "simes-closed": _clip_pvalue(
            simes_closed_p_2(
                p_unique=p_unique,
                p_shared=p_shared,
...
def _compute_unit_specific_single_unit_worker(args: Dict[str, object]) -> Dict[str, object]:
    ...
    p_simes_intersection_values = np.ones(n_genomes, dtype=float)
    p_simes_closed_values = np.ones(n_genomes, dtype=float)
    ...
    def _refresh_combined_pvalues(genome_idx: int, is_knockoff_target: bool) -> None:
        if not is_knockoff_target:
            ...
            p_simes_intersection_values[genome_idx] = 1.0
            p_simes_closed_values[genome_idx] = 1.0
        ...
        p_simes_intersection_values[genome_idx] = components["simes-intersection"]
        p_simes_closed_values[genome_idx] = components["simes-closed"]
        ...
        rows.append(
            {
                ...
                "pvalue_simes_intersection": float(p_simes_intersection_values[genome_idx]),
                "pvalue_simes_closed": float(p_simes_closed_values[genome_idx]),
