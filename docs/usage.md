...
Sample-level peptide presence is aggregated by union within each analysis unit. Every unit independently receives the same shared-peptide knockoff, unique-evidence model, presence score, p-value, and BH q-value calculation. The default unique-evidence mode is structural `auto`: it uses `empirical-background` only when eligible and otherwise `alpha-upper-bound`. The default presence p-value is the zero-unique-gated two-component Simes/closed-test result. Genomes with no matched observed peptide are outside the BH family; active genomes with no unique peptide remain in it with genome-level p-value 1. `all-samples` does not call a separate pooled scoring backend, but its empirical unique-evidence background uses a moderately more permissive calibration profile (10% initial exclusion, adaptively bounded to 5--20%) than grouped/per-sample units (5% initial exclusion, bounded to 2--20%). This prevents strong cohort-wide signals from inflating their own pooled background while retaining the same scoring formulas and output contract. The selected profile and parameters are recorded in `artifacts/run_summary.json`.
